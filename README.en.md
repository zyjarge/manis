...
The bundle is built with **PyInstaller** (py2app is no longer used — it
has compatibility issues with Python 3.12+/setuptools 80+).

```bash
# Build the .app bundle (must use a framework build of Python 3.13)
...
├── setup.py                   # py2app config (deprecated, see PACKAGING.md)
...
# Build .app bundle (see "As a macOS .app" above for the full command)
UV_PYTHON=/usr/local/Cellar/python@3.13/3.13.11_1/bin/python3.13 \
  uv run --python .venv/bin/python pyinstaller \
    --noconfirm --windowed --name manis \
    --add-data "manis/web:manis/web" main.py
